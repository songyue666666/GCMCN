...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        X = torch.unsqueeze(X,dim=1) # expand the dimensions of x
        X = X.transpose(2,3)
        with torch.no_grad():
            output = model(X)
    ...
    #     plt.plot(predict[:, i] + i * np.ones((predict.shape[0], )))
    #     plt.plot(Ytest[:, i] + i * np.ones((predict.shape[0], )))
    # plt.show()
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        for j in range(args.num_split):
            if j != args.num_split - 1:
                ...
                id = perm[j * num_sub:]
            id = torch.tensor(id).to(device).type(torch.int64)
            tx = X[:, :, id, :]
            ty = Y[:, id]
            output = model(tx,id)
            output = torch.squeeze(output)
...
parser.add_argument('--save', type=str, default='model/exchange_rate.pt',help='path to save the final model')
parser.add_argument('--optim', type=str, default='adam')
parser.add_argument('--L1Loss', type=bool, default=True)
parser.add_argument('--normalize', type=int, default=2)
parser.add_argument('--device',type=str,default='cpu',help='')#'cuda:1'
...
parser.add_argument('--adj_threshold',type=float,default=0,help='the threshold of A to made matrix sparse')
parser.add_argument('--node_dim',type=int,default=10,help='dimensions of nodes')
parser.add_argument('--dilation_exponential',type=int,default=2,help='dilation exponential')
parser.add_argument('--conv_channels',type=int,default=16,help='convolution channels')
parser.add_argument('--end_channels',type=int,default=64,help='end channels')
parser.add_argument('--in_dim',type=int,default=1,help='inputs dimension')
parser.add_argument('--seq_in_len',type=int,default=150,help='input sequence length')
parser.add_argument('--seq_out_len',type=int,default=1,help='output sequence length')
parser.add_argument('--horizon', type=int, default=3)
parser.add_argument('--layers',type=int,default=3,help='number of layers')
parser.add_argument('--max_kernelsize', type=int, default=5, help='the max size of conv kernels')

parser.add_argument('--batch_size',type=int,default=16,help='batch size')
parser.add_argument('--lr',type=float,default=0.0001,help='learning rate')
parser.add_argument('--weight_decay',type=float,default=0.00001,help='weight decay rate')

parser.add_argument('--clip',type=int,default=5,help='clip of optimizers')
...
def main():
    ...
    model = gcmcn(args.gcn_true, args.buildA_true, args.num_nodes, device, 
                  dropout=args.dropout, adj_threshold=args.adj_threshold,
                  node_dim=args.node_dim, dilation_exponential=args.dilation_exponential,
                  conv_channels=args.conv_channels, end_channels= args.end_channels,
                  seq_length=args.seq_in_len, in_dim=args.in_dim, out_dim=args.seq_out_len,
                  layers=args.layers, max_kernel_size=args.max_kernelsize, layer_norm_affline=False)
    print(model)
