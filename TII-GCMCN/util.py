...
class DataLoader(object):
    ...
    def __init__(self, file_name, train, valid, device, horizon, window, normalize=2):
        self.P = window
        self.h = horizon #the prediction steps
        fin = open(file_name)
        self.rawdat = np.loadtxt(fin, delimiter=',')
        self.dat = np.zeros(self.rawdat.shape)
        self.n, self.m = self.dat.shape # time point # number of variables
        self.normalize = 2 
        self.scale = np.ones(self.m) 
        self._normalized(normalize) # normalization
        self._split(int(train * self.n), int((train + valid) * self.n), self.n) # split the training, testing, valid dataset
    ...
    def _normalized(self, normalize):
        ...
        if (normalize == 2):
            for i in range(self.m):
                ...
                if np.max(np.abs(self.rawdat[:, i])) != 0:
